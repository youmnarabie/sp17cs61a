...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('length of formals and vals mismatched')
        def definer(formals, vals):
            if vals != nil:
                child.define(formals.first, vals.first)
                definer(formals.second, vals.second)
        definer(formals, vals)
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        eval_oper = operands.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(self, eval_oper, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            print(self, args)
            raise SchemeError('application error')
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    def helper(expressions, env):
        ...
        if evaled is not False:
            last_expr = evaled
            return helper(expressions.second, env)
        elif evaled == False:
            return False
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            if clause.first == 'else' or scheme_truep(test):
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(bound, 2, 2)
        values = Pair(scheme_eval(bound.second.first, env), values)
        formals = Pair(bound.first, formals)
