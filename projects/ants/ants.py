...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.only_queen:
                return 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        number_of_places_passed = 0
        nearest_place = self.place
        while nearest_place is not hive:
            if nearest_place.bees and number_of_places_passed >= self.min_range and number_of_places_passed <= self.max_range:
                return random_or_none(nearest_place.bees)
            else:
                nearest_place = nearest_place.entrance
                number_of_places_passed += 1
        # END Problem 3B
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        armor_after_reduction = self.armor - amount
        if armor_after_reduction <= 0:
            bees_in_place = list(self.place.bees)
            for i in bees_in_place:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.only_queen:
            ...
            while place_start != None:
                ant_to_be_doubled = place_start.ant
                if ant_to_be_doubled != None:
                    if ant_to_be_doubled not in self.doubled and ant_to_be_doubled.name != 'Queen':
                        ant_to_be_doubled.damage *= 2
                        QueenAnt.doubled.append(ant_to_be_doubled)
                    if ant_to_be_doubled.container and ant_to_be_doubled.ant != None:
                        if ant_to_be_doubled.ant.name != 'Queen' and ant_to_be_doubled.ant not in self.doubled:
                            ant_to_be_doubled.ant.damage *= 2
                            QueenAnt.doubled.append(ant_to_be_doubled.ant) 
