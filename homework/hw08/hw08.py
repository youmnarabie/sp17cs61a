...
def missing_val(lst0, lst1):
    ...
    True
    """
    return (set(lst0)).difference(set(lst1)).pop()
...
def make_s():
    ...
    """
    def rest():
        factors_2_3 = merge(scale_stream(s,2), scale_stream(s,3))
        return merge(factors_2_3, scale_stream(s,5))
    s = Stream(1, rest)
    return s
    s = Stream(1, rest)
    return s
...
def make_random_stream(seed, a, c, n):
    ...
    """
    def rest():
        return make_random_stream((a*seed+c)%n, a, c, n)
    return Stream(seed, rest)
