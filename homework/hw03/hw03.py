...
def lt_rat(x, y):
    ...
    if x == y:
        return False
    minimum = min(numer(x) / denom(x), numer(y) / denom(y))
    if minimum == numer(x) / denom(x):
        return True
    else:
        return False
...
def pingpong(n):
    ...
    True
    """
    def functioning(number, index, reversing):
        if index == n:
            return number
        elif (index % 7 == 0 or has_seven(index)) and reversing:
            return functioning(number + 1, index + 1, False)
        elif (index % 7 == 0 or has_seven(index)) and not reversing:
            return functioning(number - 1, index + 1, True)
        elif reversing:
            return functioning(number - 1, index + 1, True)
        else:
            return functioning(number + 1, index + 1, False)
    return functioning(1, 1, False)
...
def build_ones(h, branching_factor=2):
    ...
    if h==0:
        ...
        return tree(1, [build_ones(h - 1 , branching_factor) for i in range(branching_factor)])
