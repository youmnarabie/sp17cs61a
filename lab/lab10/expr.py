...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        elif self.string not in env:
            raise NameError(self.string + 'not in current env!')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        evaluated_operator = self.operator.eval(env)
        evaluated_operands = [s.eval(env) for s in self.operands]
        return evaluated_operator.apply(evaluated_operands)
